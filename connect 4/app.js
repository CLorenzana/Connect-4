...
for (var i=0, len = squares.length; i < len; i++)
    ...
    (function(index){
    //add onclick square in grid
        squares[i].onclick=function(){
        //if square below is taken
            if(squares[index + 7].classList.contains('taken')){
                if (currentPlayer === 1){
                    ...
                    //if square below not taken
                }
                else alert('cannot go here')
            }
        })(i)

//check board if win or lose
function checkBoard(){
    ...
    //take 4 values and plug into square values
    for(let y = 0; y < winningArrays.length; y++){
        ...
        //check if arrays all have the class of player-one
        ...
        //check if arrays all have the class of player-two
...
//event listener for each square to trigger checkboard functions
